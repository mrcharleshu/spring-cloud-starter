...
public class UserApplicationTests {
    ...
    @Test
    public void shouldRoundRobinOverInstancesWhenCallingServicesViaRibbon() {
        ResponseEntity<String> response1 = this.testRestTemplate.getForEntity(
                "http://localhost:" + this.port + "/hi?name=foo", String.class);
        ResponseEntity<String> response2 = this.testRestTemplate.getForEntity(
                "http://localhost:" + this.port + "/hi?name=foo", String.class);
        ResponseEntity<String> response3 = this.testRestTemplate.getForEntity(
                "http://localhost:" + this.port + "/hi?name=foo", String.class);
    ...
    private ConfigurableApplicationContext startApp(int port) {
        return SpringApplication.run(TestApplication.class,
                "--server.port=" + port, "--spring.jmx.enabled=false");
    }
